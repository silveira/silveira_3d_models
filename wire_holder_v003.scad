...
difference() {
    union() {
        ...
        // Wall between left gussets (vertical drop from tab to bridge)
        translate([0, overhang_length - wall_thickness, -gusset_height])
            cube([overhang_width, wall_thickness, gusset_height]);

        // Wall between right gussets (vertical drop from tab to bridge)
