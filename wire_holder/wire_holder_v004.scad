...
// Walls between each pair of gussets linking overhang tabs to bridge
// Walls positioned at gusset vertical edge, extending into bridge gap
...
// Wall thickness (between gusset pairs)
wall_thickness = 2;
...
difference() {
    union() {
        ...
        // Wall between left gussets (extends into bridge gap, flush with gusset vertical edge)
        translate([0, overhang_length, -gusset_height])
            cube([overhang_width, wall_thickness, gusset_height]);

        // Wall between right gussets (extends into bridge gap, flush with gusset vertical edge)
        translate([0, overhang_length + bridge_span - wall_thickness, -gusset_height])
            cube([overhang_width, wall_thickness, gusset_height]);

        // Gussets - left overhang
        translate([0, overhang_length, -gusset_height])
            mirror([0, 1, 0])
