...
// Door profile: bottom corners rounded, top corners square (open to top edge)
module door_2d(width, height, radius) {
    hull() {
        // Top-left square
...
difference() {
    ...
    // Door on the long side (front), centered horizontally, near top edge
    // Bottom corners rounded, top is open (square) to the top edge
    translate([outer_length / 2 - side_hole_width / 2, wall + 1, outer_height - side_hole_height])
        rotate([90, 0, 0])
            linear_extrude(height = wall + 2)
                door_2d(side_hole_width, side_hole_height + 1, side_hole_corner_radius);
