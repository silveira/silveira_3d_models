...
// Door profile: top corners rounded inward, bottom corners rounded outward
module door_2d(width, height, top_r, bottom_r) {
    intersection() {
        translate([0, -bottom_r])
            square([width, height + bottom_r]);
        union() {
            // Main body with rounded top corners
            hull() {
                translate([top_r, height - top_r])
                    circle(r = top_r, $fn = 32);
                translate([width - top_r, height - top_r])
                    circle(r = top_r, $fn = 32);
                square([width, height - top_r]);
            }
            // Bottom-left outward round
            circle(r = bottom_r, $fn = 32);
            // Bottom-right outward round
            translate([width, 0])
                circle(r = bottom_r, $fn = 32);
        }
    }
...
difference() {
    ...
    // Door hole on the front side: top rounded inward, bottom rounded outward
    translate([outer_length / 2 - side_hole_width / 2, wall + 1, outer_height - side_hole_height])
        rotate([90, 0, 0])
            linear_extrude(height = wall + 2)
                door_2d(side_hole_width, side_hole_height + 1, side_hole_corner_radius, side_hole_corner_radius);
